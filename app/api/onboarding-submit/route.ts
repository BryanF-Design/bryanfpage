  ...
  try {
    ...
    const form =
      body.form && typeof body.form === "object"
        ? (body.form as Record<string, string>)
        : {};
    ...
    const summaryItems = Array.isArray(body.summaryItems)
      ? (body.summaryItems as Array<Record<string, unknown>>)
      : [];
    const payment =
    ...
    if (!resendResponse.ok || resendData.error) {
      const detail =
        resendData && resendData.error
          ? resendData.error.message || JSON.stringify(resendData.error)
          : "Email service error";
      return NextResponse.json(
    ...
    return NextResponse.json(
      { error: `Onboarding submit error: ${message}` },
      { status: 500 }
    );
  }
}
